...
object ESProxy extends TwitterServer {
  ...
  protected var _client : ESClient = null
  
  premain {
      // build the client. 
	  // do it in the premain and not in the object construction to have the flags initialized
	  log.info("building ES client with hosts: " + es_hosts())
	  _client = ESClient.build(es_cluster(), es_hosts(), use_native())
  }
